...
| `S3_RETRY_BACKOFF_DURATION`            | `time.Duration` | `1s` | The retry backoff duration when the workflow init container performs retries connecting to S3.                                |
| `S3_RETRY_BACKOFF_FACTOR`              | `float` | `1.6` | The retry backoff factor when the workflow init container performs retries connecting to S3.                                  |
| `S3_RETRY_BACKOFF_JITTER`              | `float` | `0.5` | The retry backoff jitter when the workflow init container performs retries connecting to S3.                                  |
| `S3_RETRY_BACKOFF_STEPS`               | `int` | `5` | The retry backoff steps when the workflow init container performs retries connecting to S3.            |
